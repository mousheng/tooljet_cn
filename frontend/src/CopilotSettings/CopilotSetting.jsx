...
export const CopilotSetting = () => {
  ...
  const saveCopilotApiKey = async (apikey) => {
    ...
    return setTimeout(() => {
      if (isCopilotApiKeyPresent === true && !copilotWorkspaceVarId) {
        return orgEnvironmentVariableService
          ...
          .then(() => {
            ...
            toast.success('Copilot API密钥保存成功');
          })
          .catch((err) => {
            console.log(err);
            return toast.error('出了问题');
          })
          .finally(() => {
            setIsLoading(false);
      ...
      return toast.error('API密钥无效') && setIsLoading(false);
    }, 400);
  };
...
const EducativeLebel = ({ darkMode }) => {
  const title = () => {
    return (
      <>
        了解tooljet <strong style={{ fontWeight: 700, color: '#3E63DD' }}>AI copilot</strong>
      </>
    );
  };
    ...
    <Popover id="transformation-popover-container">
      <div className="transformation-popover card text-center">
        ...
        <div className="d-flex flex-column card-body">
          ...
          <p className="mb-2">
            <strong style={{ fontWeight: 700, color: '#3E63DD' }}>AI copilot</strong> 帮助您更快地编写查询它使用openAI的GPT3.5根据您的数据建议查询.
          </p>
